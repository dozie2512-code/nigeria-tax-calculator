  ...
  const INDIVIDUAL_ACCOUNTS = ['Salary', 'Dividend', 'Interest', 'Royalty', 'Share of profit'];
  ...
  const superAdminMsg = document.getElementById('superAdminMsg');
  ...
  /**
   * Check if current session has super admin access
   * @returns {boolean}
   */
  function isSuperAdmin(){
    return state.superAdmin && state.superAdmin.isActive === true;
  }

  /**
   * Activate super admin mode with session timeout
   */
  function activateSuperAdmin(){
    state.superAdmin = state.superAdmin || {};
    state.superAdmin.isActive = true;
    state.superAdmin.activatedAt = Date.now();
    
    // Set session timeout for 30 minutes of inactivity
    resetSuperAdminTimeout();
    
    saveState();
    addAudit('superadmin.activated', 'auth', null, 'Super admin access activated');
    updateAuthUI();
    
    // Reset timeout on user activity
    setupSuperAdminActivityListeners();
  }
  
  /**
   * Reset super admin timeout (called on user activity)
   */
  function resetSuperAdminTimeout(){
    if(!isSuperAdmin()) return;
    
    const SUPER_ADMIN_TIMEOUT_MS = 30 * 60 * 1000; // 30 minutes
    
    // Clear any existing timeout
    if(state.superAdmin && state.superAdmin.timeoutId){
      clearTimeout(state.superAdmin.timeoutId);
    }
    
    // Set new timeout
    state.superAdmin = state.superAdmin || {};
    state.superAdmin.timeoutId = setTimeout(()=> {
      if(isSuperAdmin()){
        deactivateSuperAdmin();
        // Show accessible notification instead of alert
        if(superAdminMsg){
          superAdminMsg.textContent = 'Super Admin session has expired due to inactivity. Please login again if needed.';
          superAdminMsg.style.color = '#b00';
          superAdminMsg.setAttribute('role', 'alert');
          if(superAdminModal) openModal(superAdminModal);
        }
      }
    }, SUPER_ADMIN_TIMEOUT_MS);
  }
  
  /**
   * Setup activity listeners to reset super admin timeout
   */
  function setupSuperAdminActivityListeners(){
    const events = ['mousedown', 'keydown', 'scroll', 'touchstart'];
    const throttledReset = throttle(resetSuperAdminTimeout, 1000); // Throttle to once per second
  ...
  function deactivateSuperAdmin(){
    if(state.superAdmin){
      ...
      if(state.superAdmin.timeoutId){
        clearTimeout(state.superAdmin.timeoutId);
        state.superAdmin.timeoutId = null;
      }
      
      state.superAdmin.isActive = false;
      state.superAdmin.deactivatedAt = Date.now();
    }
    saveState();
    addAudit('superadmin.deactivated', 'auth', null, 'Super admin access deactivated');
    updateAuthUI();
  }
  ...
  function validateSuperAdminPassword(password){
    // SECURITY WARNING: Hardcoded password for development/demo only
    // 
    // ⚠️ IMPORTANT: This uses a deliberately weak password to make it obvious
    // this is not production-ready. For any environment with real data:
    // 
    // In production, this should use environment variables or secure vault
    ...
    // }
    //
    // Development password (DO NOT use in production):
    const SUPER_ADMIN_PASSWORD = 'aaaaaa';
  ...
  function enforcePermissions(){
    ...
    const isSuperAdminActive = isSuperAdmin();
    ...
    // Always disable sensitive operations for non-super-admins
    ...
    // Mark sensitive operations as disabled for non-super-admins
  ...
  function initUI(){
    ...
    const superAdminSubmit = document.getElementById('superAdminSubmit');
    const superAdminCancel = document.getElementById('superAdminCancel');
    
    if(btnSuperAdmin){
      btnSuperAdmin.addEventListener('click', ()=> {
        if(isSuperAdmin()){
          // Already logged in as super admin - offer to logout
          if(confirm('You are currently logged in as Super Admin. Do you want to logout from Super Admin mode?')){
            deactivateSuperAdmin();
            alert('Super Admin mode deactivated.');
          }
        } else {
          // Show super admin login modal
    ...
    if(superAdminSubmit){
      superAdminSubmit.addEventListener('click', ()=> {
        const password = superAdminPassword ? superAdminPassword.value : '';
        if(!password){
          if(superAdminMsg){ 
            superAdminMsg.textContent = 'Please enter the super admin password.'; 
            superAdminMsg.style.color = '#b00'; 
            superAdminMsg.setAttribute('role', 'alert');
          }
          return;
    ...
    if (authForgotPassword) {
      authForgotPassword.addEventListener('click', async function(e) {
        e.preventDefault();
        
        const preLoginUsernameEl = document.getElementById('preLoginUsername');
        const inputValue = preLoginUsernameEl ? preLoginUsernameEl.value.trim() : '';
        
        // Helper to show messages in authLoginMsg
        function showMsg(text, isError) {
          if (authLoginMsg) {
            authLoginMsg.textContent = text;
            authLoginMsg.style.color = isError ? '#b00' : 'green';
          }
        }
        
        // Validate: empty field
        if (!inputValue) {
          showMsg('Please enter your email address above to reset your password.', true);
          return;
        }
        
        // Validate: check if it looks like an email (contains @)
        if (inputValue.indexOf('@') === -1) {
          showMsg('Please enter your email address (not username) to reset your password.', true);
          return;
        }
        ...
        if (typeof isFirebaseAuthEnabled === 'function' && isFirebaseAuthEnabled()) {
          showMsg('Sending password reset link...', false);
          ...
          if (result.success) {
            ...
            showMsg(result.error || 'Failed to send reset email.', true);
          }
        } else {
          // Fallback for local prototype mode
          showMsg('Password reset is not available in prototype mode. Please contact your admin.', true);
        }
      });
    }
  ...
  function renderFeatureVisibility(){
    ...
    if (isIndividual) {
      ...
      // Restrict actions - only show Revenue for Individuals
      Array.from(txnAction.options).forEach(opt => {
        if (opt.value && opt.value !== 'Revenue') {
          opt.style.display = 'none';
        } else if (opt.value === 'Revenue') {
          opt.style.display = '';
        }
      });
  ...
  function clearAllData(){
    if(!isSuperAdmin()){
      ...
      const msg = 'Super Admin access required to clear all data. Please login as Super Admin first.';
      if(superAdminMsg && superAdminModal){
        superAdminMsg.textContent = msg;
        superAdminMsg.style.color = '#b00';
        superAdminMsg.setAttribute('role', 'alert');
        openModal(superAdminModal);
      } else {
  ...
  function ensureSelection(){
    ...
    businessSelect.addEventListener('change', ()=> { selectedBizId = businessSelect.value; populateAccountSelect(); renderAll(); renderFeatureVisibility(); updateReportTabVisibility(); });
    ...
    if(asset.classification === 'Chargeable' && (isPropertyAsset(asset) || isShareAsset(asset))) {
      ...
      if(chargeableGain < 0) {
        ...
        if(netGainAfterOffset > 0 && !isShareExempt) {
          if(b.entity === 'Individual' || b.entity === 'Enterprise') {
            // Individual or Enterprise: include netGain in taxable profit
  ...
  function renderTaxSummary(){
    ...
    for(const g of chargeableGains){
      ...
      if(b.entity === 'Company'){
        ...
        // For Individual/Enterprise: gains included in taxable profit unless exempt
    ...
    if(b.entity === 'Company' && b.citEnabled){
      ...
      // Deduct WHT receivable only if CIT was actually computed (turnover above threshold and CIT > 0)
      if(citCharge > 0) {
    ...
    // PIT for Individual (compute properly with statutory deductions and WHT receivable)
    if(isIndividual && b.pitEnabled){
      ...
      pitCharge = Math.max(0, pitLiability - whtReceivable);
    }
    ...
    if(isIndividual){
      ...
      html += `<div class="small-note">PIT is computed on taxable income (gross income less statutory deductions). WHT receivable is then deducted from the PIT liability.</div>`;
    } else {
  ...
  function renderReport(name){
    ...
    if(name === 'vatReport'){
      ...
      html += `<tr><td>Revenue</td><td style="text-align:right;">${fmt(revenue)}</td></tr>`;
      html += `<tr><td>Cost of Sales / Expenses</td><td style="text-align:right;">(${fmt(expenses - depreciation)})</td></tr>`;
      html += `<tr><td>Depreciation</td><td style="text-align:right;">(${fmt(depreciation)})</td></tr>`;
      html += `<tr style="background:#f2f6fa;"><td><strong>Profit/(Loss) for the Year</strong></td><td style="text-align:right;"><strong>${fmt(grossProfit)}</strong></td></tr>`;
      html += `</tbody></table>`;
      ...
      html += `<tr><td style="padding-left:20px;">Other Assets</td><td style="text-align:right;">${fmt(totalAssets - fixedAssetsNBV - inventoryValue)}</td></tr>`;
      ...
      html += `<tr><td style="padding-left:20px;">Total Liabilities</td><td style="text-align:right;">${fmt(totalLiabilities)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Share Capital</td><td style="text-align:right;">${fmt(totalCapital)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Retained Earnings</td><td style="text-align:right;">${fmt(grossProfit)}</td></tr>`;
      html += `<tr style="background:#f2f6fa;"><td><strong>Total Liabilities & Capital</strong></td><td style="text-align:right;"><strong>${fmt(totalLiabilities + totalCapital + grossProfit)}</strong></td></tr>`;
      html += `</tbody></table>`;
  ...
  function deleteUser(id){
    if(!isSuperAdmin()){
      // Use accessible notification instead of alert
      const msg = 'Super Admin access required to delete users. Please login as Super Admin first.';
      if(superAdminMsg && superAdminModal){
        superAdminMsg.textContent = msg;
        superAdminMsg.style.color = '#b00';
        superAdminMsg.setAttribute('role', 'alert');
  ...
  (function initOfferCountdownAndPricing() {
    ...
    const PLANS = {
      business: {
        ...
        addonLink: 'https://buy.stripe.com/addon-selffile-business'
      },
      company: {
        fullPrice: 80000,
        offerPrice: 60000,
        ...
        addonLink: 'https://buy.stripe.com/addon-selffile-company'
      },
      enterprise: {
        fullPrice: 150000,
        offerPrice: 100000,
    ...
    if (forgotSubmit) {
      forgotSubmit.addEventListener('click', async function() {
        ...
        if (typeof isFirebaseAuthEnabled === 'function' && isFirebaseAuthEnabled()) {
          ...
          // Fallback to local prototype behavior
...
document.addEventListener('DOMContentLoaded', function () {
  var forgotLink = document.getElementById('authForgotPassword');
  var loginMsg = document.getElementById('authLoginMsg');
  ...
  forgotLink.addEventListener('click', async function (e) {
    ...
    // Ask for email — replace with a proper modal/input if you prefer
    var email = prompt('Enter the email address for your account:');
    if (!email) {
      return;
    }

    // If Firebase not enabled, show helpful message
    if (!isFirebaseAuthEnabled()) {
      loginMsg.textContent = 'Password reset unavailable: Firebase not configured. Check FIREBASE_ENABLED and firebaseConfig.';
      return;
    }

    // Call helper that uses firebaseAuth.sendPasswordResetEmail
    try {
      var res = await firebaseSendPasswordReset(email);
      if (res.success) {
        loginMsg.textContent = 'Password reset email sent. Check your inbox (and spam).';
      } else {
        loginMsg.textContent = 'Reset failed: ' + (res.error || 'Unknown error');
      }
    } catch (err) {
      console.error('Forgot password error', err);
      loginMsg.textContent = 'An unexpected error occurred. See console.';
    }
  });
});
