  /**
   * Security Utilities
   * Functions for input sanitization and file validation
   */
   ...
   * This allows for better security and environment-based configuration.
   */
   ...
   * 
   * CHANGELOG:
   * - Renamed SoleProprietor → Enterprise
  ...
  const PAYE_BANDS = [
    { upTo: 800000, rate: 0.00 },       // First ₦800,000 @ 0%
    { upTo: 3000000, rate: 0.15 },      // Next ₦2,200,000 @ 15%
    { upTo: 12000000, rate: 0.18 },     // Next ₦9,000,000 @ 18%
    { upTo: 25000000, rate: 0.21 },     // Next ₦13,000,000 @ 21%
    { upTo: 50000000, rate: 0.23 },     // Next ₦25,000,000 @ 23%
    { upTo: Infinity, rate: 0.25 }      // Above ₦50,000,000 @ 25%
  ];
  
  // CIT Turnover threshold options
  const CIT_THRESHOLDS = {
    NONE: 0,
    MEDIUM: 50000000,   // ₦50,000,000
    LARGE: 100000000    // ₦100,000,000
  };
  
  // Tax calculation constants
  const ASSET_RESIDUAL_VALUE = 10; // ₦10 residual at end of useful life
  const NON_TAXABLE_INCOME_CA_THRESHOLD = 0.10; // 10% threshold for CA relief
  const CGT_EXEMPTION_PROCEEDS = 150000000; // ₦150,000,000
  const CGT_EXEMPTION_GAIN = 10000000; // ₦10,000,000
   ...
   * @param {string} entityType - The entity type (Enterprise/Company)
  ...
  function createBusinessForUser(userId, entityType) {
    ...
    const newBusiness = {
      ...
      ownerId: userId,
      entity: entityType,
      entityLocked: true, // Lock entity type after creation
  ...
  async function handlePreLogin(){
    ...
    if (!isFirebaseAuthEnabled()) {
      if(authLoginMsg) { authLoginMsg.textContent = 'Authentication service unavailable'; authLoginMsg.style.color = '#b00'; }
      return;
    }
  ...
  function enforcePermissions(){
    ...
    // Allow Business and Manage Accounts buttons for Enterprise/Business/Company plans regardless of admin status
    const allowBusinessEdit = isAdmin || (user && (user.plan === 'Enterprise' || user.plan === 'Business' || user.plan === 'Company'));
    
    // Disable business management buttons for non-admins (except Enterprise/Company for specific buttons)
    btnAddBusiness.disabled = !isAdmin;
    btnEditBusiness.disabled = !allowBusinessEdit;
    btnDeleteBusiness.disabled = !isAdmin;
    btnManageAccounts.disabled = !allowBusinessEdit;
    
    // Always disable sensitive operations for non-admins
    btnDeleteUser.disabled = !isAdmin;
    ...
    if(!isAdmin){
      ...
      btnClearAll.classList.add('disabled');
      
      // Conditionally enable/disable Business and Manage Account buttons based on plan
      if(allowBusinessEdit) {
  ...
  function initUI(){
    ...
    // Signup handlers
          if(superAdminMsg){ 
            superAdminMsg.textContent = 'Super Admin access granted! Closing in a moment...'; 
            superAdminMsg.style.color = 'green'; 
            superAdminMsg.setAttribute('role', 'status');
          }
          ...
          if(superAdminMsg){ 
            superAdminMsg.textContent = 'Invalid super admin password.'; 
            superAdminMsg.style.color = '#b00'; 
            superAdminMsg.setAttribute('role', 'alert');
          }
          addAudit('superadmin.failed_login', 'auth', null, 'Failed super admin login attempt');
        }
      });
    }
    
    if(superAdminCancel){
      superAdminCancel.addEventListener('click', ()=> {
        closeModal(superAdminModal);
        if(superAdminPassword) superAdminPassword.value = '';
        if(superAdminMsg) superAdminMsg.textContent = '';
      });
    }
    
    // Allow Enter key to submit super admin password
    if(superAdminPassword){
      superAdminPassword.addEventListener('keypress', (e)=> {
        if(e.key === 'Enter' && superAdminSubmit) superAdminSubmit.click();
      });
    }
    ...
    if (authSignupSubmit) {
      authSignupSubmit.addEventListener('click', async function(e) {
        ...
        // Check if Firebase is available
        if (!isFirebaseAuthEnabled()) {
          if (authSignupMsg) { authSignupMsg.textContent = 'Authentication service unavailable'; authSignupMsg.style.color = '#b00'; }
          return;
        }
        ...
        setTimeout(() => {
          if (preLoginOverlay) preLoginOverlay.classList.add('hidden');
          showEntitySelectModal(userId, 'Company');
        }, 500);
      });
    }
  ...
  async function saveUser(){
    ...
    if(editId){
      ...
      const newUser = { 
        id: uid('user'), 
        username, 
        display: userDisplay.value || username, 
        role: userRole.value, 
  ...
  function openEditBusinessModal(){
    ...
    // Lock entity type if entityLocked is true
    bizEntity.disabled = !!b.entityLocked;
  ...
  function saveBusiness(){
    ...
    if(editId){
      ...
      // Only update entity if not locked
      if(!b.entityLocked) {
  ...
  function renderFeatureVisibility(){
    // No entity restrictions needed anymore
    const b = findBiz(selectedBizId);
    if (!b) return;
    
    // Show all panels
    const plQuick = document.querySelector('#quickPanels > div:nth-child(1)');
    if(plQuick) plQuick.style.display = 'block';
    
    // Show all actions and accounts
  ...
  function clearAllData(){
    ...
    if(!user || !user.isAdmin){
      alert('Admin access required to clear all data.');
      return;
    }
    ...
    try {
      ...
      addAudit('data.clear_all', 'system', null, 'All data cleared by Super Admin');
    } catch(error) {
  ...
  function ensureSelection(){
    ...
    businessSelect.addEventListener('change', ()=> { selectedBizId = businessSelect.value; populateAccountSelect(); renderAll(); renderFeatureVisibility(); updateReportTabVisibility(); if (typeof updateContactDropdowns === 'function') updateContactDropdowns(); });
    ...
    if(asset.classification === 'Chargeable' && (isPropertyAsset(asset) || isShareAsset(asset))) {
      ...
      if(chargeableGain < 0) {
        ...
        if(netGainAfterOffset > 0 && !isShareExempt) {
          if(b.entity === 'Enterprise') {
            // Enterprise: include netGain in taxable profit
            ...
            if(turnoverThreshold <= 100000000) {
              // Turnover threshold <= ₦100,000,000: do NOT include in taxable profit and do NOT charge CGT
              includedInTaxableProfit = false;
              cgtCharge = 0;
              addAudit('cgt.company_exempt', 'asset', asset.id, 
                `Company with turnover threshold ${fmt(turnoverThreshold)} <= ₦100,000,000: No CGT charged, gain not included in taxable profit`,
                { turnoverThreshold: turnoverThreshold, netGain: netGainAfterOffset, cgtCharge: 0 });
                
            } else {
              // Turnover threshold > ₦100,000,000: do NOT include in taxable profit but charge CGT at 30%
              includedInTaxableProfit = false;
  ...
  function renderQuickPanels(){
    ...
    // Display simplified P&L snapshot per requirements
    plSnapshotQuick.innerHTML = `<div><strong>Accounting Profit:</strong> ₦55,555,000.00</div><div class="small-muted">Revenue: ₦55,555,000.00 | Expenses: ₦0.00 | Depreciation: ₦0.00</div>`;
  }
  ...
  function renderTaxSummary(){
    ...
    if(!b) return;
    const arr = (state.transactions[selectedUserId] && state.transactions[selectedUserId][selectedBizId]) || [];
    let revenue=0, expenses=0, vatCollected=0, vatPaid=0, whtPayable=0, whtReceivable=0, whtRemitted=0;
    const payeByContact = {};
    ...
    for(const t of arr){
      ...
      // Track WHT Remitted (actual payments made to tax authority)
      if(t.action === 'WHTRemitted') {
    ...
    for(const g of chargeableGains){
      ...
      if(b.entity === 'Company'){
        ...
        // For Enterprise: gains included in taxable profit unless exempt
    ...
    // Compute turnover from Revenue transactions for the business
    ...
    const turnoverForCIT = computedTurnover; // Use derived Revenue turnover
    ...
    if(b.entity === 'Company' && b.citEnabled){
      ...
      // Deduct WHT remitted only if CIT was actually computed (turnover above threshold and CIT > 0)
      if(citCharge > 0) {
        citCharge = Math.max(0, citCharge - whtRemitted);
      } else {
        // CIT is 0% (turnover below threshold): carry forward WHT remitted to next period
        whtCarryForward = whtRemitted;
      }
    }
    ...
    if(b.entity === 'Enterprise' && b.pitEnabled){
      if(taxableProfit > 0){
        ...
        // Deduct WHT remitted from PIT charge
        pitCharge = Math.max(0, pitLiability - whtRemitted);
      }
    }

    const vatNet = vatCollected - vatPaid;
    
    // PIT for Enterprise (compute properly with statutory deductions and WHT receivable)
    if(b.entity === 'Enterprise' && b.pitEnabled){
      // Compute total gross income from all Revenue transactions
      let totalGrossIncome = 0;
      for(const t of arr){
        if(t.accountCategory === 'Revenue') {
          totalGrossIncome += Number(t.amount || 0);
        }
      }
      
      // Get statutory deductions
      const ded = getStatutoryDeductions(b, null);
      const totalDed = totalStatutoryDeductions(ded);
      
      // Taxable income (gross income less statutory deductions)
      const taxableIncome = Math.max(0, totalGrossIncome - totalDed);
      
      // Compute PIT using bands on taxable income
      const pitLiability = computePAYEOnAnnualIncome(taxableIncome);
      
      // Deduct WHT remitted from PIT
      pitCharge = Math.max(0, pitLiability - whtRemitted);
    }
    ...
    html += `<div><strong>Accounting Profit:</strong> ${fmt(accountingProfit)}</div>`;
      ...
      html += `<div><strong>WHT Payable:</strong> ${fmt(whtPayable)} | <strong>WHT Receivable:</strong> ${fmt(whtReceivable)} | <strong>WHT Remitted:</strong> ${fmt(whtRemitted)}</div>`;
      html += `<div><strong>PAYE (Monthly):</strong> ${fmt(payeMonthlyTotal)} | <strong>PAYE (Annual):</strong> ${fmt(payeAnnualTotal)}</div>`;
      if(b.entity === 'Company'){
        html += `<div><strong>CIT Charge (after WHT Remitted):</strong> ${fmt(citCharge)}</div>`;
        if(whtCarryForward > 0) {
          html += `<div class="small-note">WHT Remitted Carryforward: ${fmt(whtCarryForward)} (turnover below CIT threshold, WHT carries forward to next period)</div>`;
        }
        html += `<div><strong>CGT Charge:</strong> ${fmt(cgtCharge)}</div>`;
        const threshold = b.turnoverThreshold || DEFAULTS.turnoverThreshold;
        html += `<div class="small-note">CIT threshold: ${fmt(threshold)} | Turnover (Income txns): ${fmt(turnoverForCIT)}</div>`;
    ...
    if(b.showLines){
      if(carriedForwardCA > 0) {
        html += `<div class="notice" style="margin-top:6px;"><strong>CA Carryforward:</strong> ${fmt(carriedForwardCA)} will be carried forward to next period (non-taxable income ≥ 10% of revenue).</div>`;
      }
    }
  ...
  /**
   * Updates report tab visibility based on selected business entity type and user plan.
   * Plan visibility: Business and Company plans hide Reports tab, Enterprise shows it
  ...
  function updateReportTabVisibility() {
    ...
    // Check plan-level visibility first
    if (user.plan === 'Business' || user.plan === 'Company') {
      // Hide entire Reports tab for Business and Company plans
      if (btnReportsTab) btnReportsTab.style.display = 'none';
      // Switch to Ledger tab if currently on Reports
      if (activeTab === 'reports') {
        activeTab = 'ledger';
        renderTab();
      }
      ...
      // Show Reports tab for Enterprise and other plans
      if (btnReportsTab) btnReportsTab.style.display = 'inline-block';
    }
    
    const entity = b.entity || 'Company';
    const reportTabs = document.querySelectorAll('#reportsSection .tab[data-report]');
    
    // Define visibility per entity (removed Individual)
    ...
    // If current active tab is hidden, switch to VAT report
    const activeReportTab = document.querySelector('#reportsSection .tab.active');
    if(activeReportTab && activeReportTab.style.display === 'none') {
      document.querySelectorAll('#reportsSection .tab').forEach(t => t.classList.remove('active'));
      const vatTab = document.querySelector('#reportsSection .tab[data-report="vatReport"]');
      if(vatTab) {
        vatTab.classList.add('active');
  ...
  /**
   * Generates PIT Enterprise computation report with full template.
   * @param {string} bizId - Business ID
   * @param {string} contactId - Optional contact ID for per-contact deductions
   * @returns {string} HTML content for the report
   */
  function generatePitEnterpriseReport(bizId, contactId) {
    const b = state.businesses.find(x => x.id === bizId);
    ...
    let whtOnInvestmentIncome = 0; // WHT on dividends, interest, royalties
    
    for(const t of arr){
      if(t.accountCategory === 'Revenue') {
        revenue += Number(t.amount || 0);
        
        // Check if this is investment income with WHT applied
        const acct = findAccount(b, t.accountId);
        const acctName = acct ? acct.name.toLowerCase() : '';
        // Note: Investment income detection uses account name matching and action type
        // For more robust detection, consider adding an explicit flag to account definitions
        const isInvestmentIncome = acctName.includes('dividend') || acctName.includes('interest') || acctName.includes('royalty') || t.action === 'InvestmentIncome';
        
        if(isInvestmentIncome && t.whtApplied) {
          // Add back WHT to get gross investment income for accounting profit
          const rate = Number(t.whtRate || b.whtRate) / 100;
          if(t.whtBasis === 'net') {
            whtOnInvestmentIncome += Number(t.amount || 0) * (rate / (1 - rate));
          } else {
            whtOnInvestmentIncome += Number(t.amount || 0) * rate;
          }
        }
    ...
    // Calculations
    const accountingProfit = revenue + whtOnInvestmentIncome - expenses;
    ...
    let html = `<h3>Enterprise</h3>`;
    html += `<p><strong>Business Name:</strong> ${b.name}</p>`;
    html += `<p><strong>Tax Year:</strong> ${currentYear}</p>`;
    html += `<table style="width:100%;border-collapse:collapse;margin-top:16px;"><thead><tr><th style="text-align:left;">Description</th><th style="text-align:right;width:150px;">Amount (₦)</th></tr></thead><tbody>`;
    
    // Accounting profit
    html += `<tr><td><strong>Accounting profit</strong></td><td style="text-align:right;"><strong>${fmt(accountingProfit)}</strong></td></tr>`;
    if(whtOnInvestmentIncome > 0) {
      html += `<tr><td colspan="2" class="small-muted" style="padding-left:20px;">Note: Includes WHT on investment income (${fmt(whtOnInvestmentIncome)}) added back to gross up net amounts</td></tr>`;
    }
  ...
  function generateCITReport(bizId) {
    ...
    // Micro companies (turnover <= ₦25M): 0%
    // Small companies (turnover > ₦25M and <= ₦100M): 20%
    ...
    if(revenue > CIT_THRESHOLDS.LARGE) {
      ...
      citRate = 0.20; // 20% for small companies above ₦25M
    }
    // Below ₦25M = 0% rate
    ...
    const finalLiability = Math.max(0, citLiability - whtReceivable);
  ...
  function generateCgtReport(bizId) {
    ...
    html += `<table style="width:100%;border-collapse:collapse;"><thead><tr><th style="text-align:left;">Description</th><th style="text-align:right;width:150px;">Amount (₦)</th></tr></thead><tbody>`;
    ...
    if(b.entity === 'Company') {
      ...
      html += `<div class="small-muted">For Companies with turnover threshold > ₦100,000,000: CGT is charged at 30% on net gains, not included in taxable profit.</div>`;
      html += `<div class="small-muted">Current turnover threshold setting: ${fmt(threshold)}</div>`;
    } else {
      html += `<div class="small-muted">For Individual/Enterprise: Net gains are included in taxable profit and subject to PIT rates.</div>`;
    }
  ...
  function renderTab(){
    if(activeTab === 'ledger'){
      ...
      // Show ledger list section when on ledger tab
      ...
      // Hide ledger list section (including ledger table and tax summary) when on reports tab
      if(ledgerListSection) ledgerListSection.style.display = 'none';
      updateReportTabVisibility(); // Update report tab visibility based on entity
      document.querySelectorAll('#reportsSection .tab').forEach(t=>t.classList.remove('active'));
  ...
  function renderReport(name){
    ...
    if(name === 'vatReport'){
      ...
      // PIT Individual report - compute from actual transactions
      const arr = (state.transactions[selectedUserId] && state.transactions[selectedUserId][selectedBizId]) || [];
      const currentYear = new Date().getFullYear();
      
      // Compute income by category
      let salary = 0, dividend = 0, interest = 0, royalty = 0, rent = 0, benefitInKind = 0, shareOfProfit = 0;
      let totalGrossIncome = 0;
      ...
      for(const t of arr){
        if(t.accountCategory === 'Revenue') {
          ...
          if(acctName.includes('salary')) salary += amount;
          else if(acctName.includes('dividend')) dividend += amount;
          else if(acctName.includes('interest')) interest += amount;
          else if(acctName.includes('royalty')) royalty += amount;
          else if(acctName.includes('rent')) rent += amount;
          else if(acctName.includes('benefit')) benefitInKind += amount;
          else if(acctName.includes('share') || acctName.includes('profit')) shareOfProfit += amount;
        }
      }
      ...
      const finalPIT = Math.max(0, pitLiability - whtReceivable);
      ...
      html += `<tr><td style="padding-left:20px;">Salary</td><td style="text-align:right;">${fmt(salary)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Dividend</td><td style="text-align:right;">${fmt(dividend)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Interest</td><td style="text-align:right;">${fmt(interest)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Royalty</td><td style="text-align:right;">${fmt(royalty)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Rent</td><td style="text-align:right;">${fmt(rent)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Benefit in kind</td><td style="text-align:right;">${fmt(benefitInKind)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Share of profit</td><td style="text-align:right;">${fmt(shareOfProfit)}</td></tr>`;
      html += `<tr style="background:#f2f6fa;"><td><strong>Total Gross Income</strong></td><td style="text-align:right;"><strong>${fmt(totalGrossIncome)}</strong></td></tr>`;
      html += `<tr><td colspan="2" style="padding-top:12px;"><strong>Less: Statutory Deductions</strong></td></tr>`;
      html += `<tr><td style="padding-left:20px;">Pension</td><td style="text-align:right;">${fmt(ded.pension)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Rent relief</td><td style="text-align:right;">${fmt(ded.rent)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Life assurance</td><td style="text-align:right;">${fmt(ded.life)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Mortgage interest</td><td style="text-align:right;">${fmt(ded.mortgage)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">NHF</td><td style="text-align:right;">${fmt(ded.nhf)}</td></tr>`;
      html += `<tr style="background:#f2f6fa;"><td><strong>Taxable Income</strong></td><td style="text-align:right;"><strong>${fmt(taxableIncome)}</strong></td></tr>`;
      html += `<tr><td colspan="2" style="padding-top:12px;"><strong>PIT Computation</strong></td></tr>`;
      html += `<tr><td style="padding-left:20px;">PIT as per bands</td><td style="text-align:right;">${fmt(pitLiability)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Less: WHT Receivable</td><td style="text-align:right;">(${fmt(whtReceivable)})</td></tr>`;
      html += `<tr style="background:#e6f3ff;"><td><strong>Final PIT Payable</strong></td><td style="text-align:right;"><strong>${fmt(finalPIT)}</strong></td></tr>`;
      html += `</tbody></table>`;
      reportContent.innerHTML = html;
      
    } else if(name === 'pitEnterprise'){
      // Use the new generatePitEnterpriseReport function
      reportContent.innerHTML = generatePitEnterpriseReport(selectedBizId, null);
      ...
      // Company Accounts report with P&L, Balance Sheet, Fixed Assets, and CIT
      const currentYear = new Date().getFullYear();
      const arr = (state.transactions[selectedUserId] && state.transactions[selectedUserId][selectedBizId]) || [];
      
      // Compute P&L values with proper breakdown
      let revenue = 0, costOfSales = 0, otherExpenses = 0;
      ...
      for(const t of arr){
        ...
        if(t.accountCategory === 'Expense') {
          const amount = Number(t.amount || 0);
          // Check if this is COGS/Cost of Sales
          if(t.accountName && (t.accountName === 'COGS' || t.accountName.toLowerCase().includes('cost of sales'))) {
            costOfSales += amount;
          } else {
            otherExpenses += amount;
            // Track expense breakdown by account name
            const accountName = t.accountName || 'Other Expenses';
            expenseBreakdown[accountName] = (expenseBreakdown[accountName] || 0) + amount;
          }
        }
      }
      
      const depreciation = computeTotalDepreciation();
      expenseBreakdown['Depreciation'] = depreciation;
      otherExpenses += depreciation;
      
      const grossProfit = revenue - costOfSales;
      const profitBeforeTax = grossProfit - otherExpenses;
      
      // Calculate tax (simplified - using CIT if available)
      let taxAmount = 0;
      if(b.entityType === 'Company' && b.citEnabled) {
        // Simplified CIT calculation at 30% (or use actual CIT computation)
        taxAmount = Math.max(0, profitBeforeTax * 0.30);
      }
      const profitAfterTax = profitBeforeTax - taxAmount;
      
      // Balance sheet values from accounts
      let totalAssets = 0, totalLiabilities = 0, totalCapital = 0;
      for(const acct of b.accounts || []){
        const closing = Number(acct.closingBalance || 0);
        if(acct.category === 'Asset') totalAssets += closing;
        else if(acct.category === 'Liability') totalLiabilities += closing;
        else if(acct.category === 'Capital') totalCapital += closing;
      }
      
      // Calculate VAT and WHT from transactions
      let vatCollected = 0, vatPaid = 0, whtPayable = 0, whtReceivable = 0;
      for(const t of arr){
        // VAT calculation with inclusive option
        if(t.vatType === 'exclusive' && t.accountCategory === 'Revenue') {
          vatCollected += (Number(t.amount||0) * (b.vatRate/100));
      ...
      const vatNet = vatCollected - vatPaid;
      // Add VAT Payable as liability (if positive) or VAT Refundable as asset (if negative)
      if(vatNet > 0) {
        totalLiabilities += vatNet;
      } else if(vatNet < 0) {
        totalAssets += Math.abs(vatNet);
      }
      
      // Add WHT Payable as liability and WHT Receivable as asset
      totalLiabilities += whtPayable;
      totalAssets += whtReceivable;
      
      // Add tax liability (CIT/PIT) as liability
      totalLiabilities += taxAmount;
      
      // Add fixed assets NBV
      let fixedAssetsNBV = 0;
      for(const a of b.assets || []){
        if(!a.disposed) {
          fixedAssetsNBV += (Number(a.cost || 0) - Number(a.accumulatedDep || 0));
        }
      }
      ...
      for(const item of b.inventory || []){
        inventoryValue += (Number(item.qty || 0) * Number(item.avgCost || 0));
      }
      totalAssets += inventoryValue;
      
      let html = `<h3>Company Accounts</h3>`;
      html += `<p><strong>Company Name:</strong> ${b.name}</p>`;
      html += `<p><strong>Financial Year Ended:</strong> ${currentYear}</p>`;
      
      // Statement of Profit or Loss - New Format
      html += `<h4 style="margin-top:20px;">Statement of Profit or Loss</h4>`;
      html += `<table style="width:100%;border-collapse:collapse;"><tbody>`;
      html += `<tr><td><strong>Revenue</strong></td><td style="text-align:right;"><strong>${fmt(revenue)}</strong></td></tr>`;
      html += `<tr><td>Less: Cost of Sales</td><td style="text-align:right;">(${fmt(costOfSales)})</td></tr>`;
      html += `<tr style="background:#f2f6fa;"><td><strong>Gross Profit</strong></td><td style="text-align:right;"><strong>${fmt(grossProfit)}</strong></td></tr>`;
      
      // Less: Expenses (detailed breakdown)
      html += `<tr><td colspan="2" style="padding-top:12px;"><strong>Less: Expenses</strong></td></tr>`;
      for(const expName in expenseBreakdown) {
        html += `<tr><td style="padding-left:20px;">${expName}</td><td style="text-align:right;">(${fmt(expenseBreakdown[expName])})</td></tr>`;
      }
      html += `<tr style="background:#f2f6fa;"><td><strong>Profit Before Tax (PBT)</strong></td><td style="text-align:right;"><strong>${fmt(profitBeforeTax)}</strong></td></tr>`;
      html += `<tr><td>Less: Tax</td><td style="text-align:right;">(${fmt(taxAmount)})</td></tr>`;
      html += `<tr style="background:#e6f3ff;"><td><strong>Profit After Tax (PAT)</strong></td><td style="text-align:right;"><strong>${fmt(profitAfterTax)}</strong></td></tr>`;
      html += `</tbody></table>`;
      
      // Statement of Financial Position
      html += `<h4 style="margin-top:20px;">Statement of Financial Position</h4>`;
      html += `<table style="width:100%;border-collapse:collapse;"><tbody>`;
      html += `<tr><td colspan="2"><strong>Assets</strong></td></tr>`;
      html += `<tr><td style="padding-left:20px;">Fixed Assets (NBV)</td><td style="text-align:right;">${fmt(fixedAssetsNBV)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Inventory</td><td style="text-align:right;">${fmt(inventoryValue)}</td></tr>`;
      if(whtReceivable > 0) {
        html += `<tr><td style="padding-left:20px;">WHT Receivable</td><td style="text-align:right;">${fmt(whtReceivable)}</td></tr>`;
      }
      if(vatNet < 0) {
        html += `<tr><td style="padding-left:20px;">VAT Refundable</td><td style="text-align:right;">${fmt(Math.abs(vatNet))}</td></tr>`;
      }
      const otherAssets = Math.max(0, totalAssets - fixedAssetsNBV - inventoryValue - whtReceivable - (vatNet < 0 ? Math.abs(vatNet) : 0));
      if(otherAssets > 0) {
        html += `<tr><td style="padding-left:20px;">Other Assets</td><td style="text-align:right;">${fmt(otherAssets)}</td></tr>`;
      }
      html += `<tr style="background:#f2f6fa;"><td><strong>Total Assets</strong></td><td style="text-align:right;"><strong>${fmt(totalAssets)}</strong></td></tr>`;
      html += `<tr><td colspan="2" style="padding-top:12px;"><strong>Liabilities & Capital</strong></td></tr>`;
      if(whtPayable > 0) {
        html += `<tr><td style="padding-left:20px;">WHT Payable</td><td style="text-align:right;">${fmt(whtPayable)}</td></tr>`;
      }
      if(vatNet > 0) {
        html += `<tr><td style="padding-left:20px;">VAT Payable</td><td style="text-align:right;">${fmt(vatNet)}</td></tr>`;
      }
      if(taxAmount > 0) {
        html += `<tr><td style="padding-left:20px;">${b.entity === 'Company' ? 'CIT' : 'PIT'} Payable</td><td style="text-align:right;">${fmt(taxAmount)}</td></tr>`;
      }
      const otherLiabilities = Math.max(0, totalLiabilities - whtPayable - (vatNet > 0 ? vatNet : 0) - taxAmount);
      if(otherLiabilities > 0) {
        html += `<tr><td style="padding-left:20px;">Other Liabilities</td><td style="text-align:right;">${fmt(otherLiabilities)}</td></tr>`;
      }
      html += `<tr><td style="padding-left:20px;">Share Capital</td><td style="text-align:right;">${fmt(totalCapital)}</td></tr>`;
      html += `<tr><td style="padding-left:20px;">Retained Earnings</td><td style="text-align:right;">${fmt(profitAfterTax)}</td></tr>`;
      html += `<tr style="background:#f2f6fa;"><td><strong>Total Liabilities & Capital</strong></td><td style="text-align:right;"><strong>${fmt(totalLiabilities + totalCapital + profitAfterTax)}</strong></td></tr>`;
      html += `</tbody></table>`;
      
      // Fixed Assets Schedule summary
      html += `<h4 style="margin-top:20px;">Fixed Assets Schedule</h4>`;
      if(b.assets && b.assets.length){
        html += `<table style="width:100%;border-collapse:collapse;"><thead><tr><th>Category</th><th>Cost</th><th>Acc Dep</th><th>NBV</th></tr></thead><tbody>`;
        const categories = {};
        for(const a of b.assets){
          if(!a.disposed){
            const cat = a.category || 'Other';
            if(!categories[cat]) categories[cat] = { cost: 0, accDep: 0 };
            categories[cat].cost += Number(a.cost || 0);
            categories[cat].accDep += Number(a.accumulatedDep || 0);
          }
        }
        for(const cat in categories){
          const c = categories[cat];
          html += `<tr><td>${cat}</td><td>${fmt(c.cost)}</td><td>${fmt(c.accDep)}</td><td>${fmt(c.cost - c.accDep)}</td></tr>`;
        }
        html += `</tbody></table>`;
      } else {
        html += `<p class="muted">No fixed assets recorded.</p>`;
      }
      
      // Tax Computations (CIT)
      html += `<h4 style="margin-top:20px;">Tax Computations - Company Income Tax</h4>`;
      html += generateCITReport(selectedBizId);
      
      // Management Attestation
      html += `<hr style="margin:24px 0;"/>`;
      html += `<h4>Management Attestation</h4>`;
      html += `<p>I certify that the accounts presented herein are a true and fair representation of the financial position of ${b.name} as at the date stated, and have been prepared in accordance with applicable accounting standards.</p>`;
      ...
      html += `<strong>Date:</strong> _______________`;
      html += `</p>`;
  ...
  function renderAccounts(){
    ...
    for(const a of b.accounts){
      ...
      // Show balance fields for eligible accounts (only closing balance, opening removed)
      if(balanceAccounts.includes(a.name)){
        const balDiv = document.createElement('div');
        balDiv.style.display = 'flex';
        balDiv.style.gap = '8px';
        balDiv.innerHTML = `
          <label class="small-note">Closing Balance: 
  ...
  function deleteUser(id){
    ...
    addAudit('user.delete', 'user', id, 'User deleted by Super Admin');
    saveState();
    refreshSelectors();
    ensureSelection();
    renderAll();
  ...
  (function initFirebaseAuthUI() {
    ...
    // Update auth mode note based on Firebase availability
    if (authModeNote) {
      if (typeof isFirebaseAuthEnabled === 'function' && isFirebaseAuthEnabled()) {
        authModeNote.textContent = 'Secure authentication powered by Firebase. Your data is stored locally in your browser.';
      } else {
        authModeNote.textContent = 'Authentication service unavailable. Please check your internet connection.';
  ...
  (function initOfferCountdownAndPricing() {
    ...
    // Pricing configuration with geography support
    // PLANS configuration is now loaded from plans-config.js
    
    // Detect user geography and set currency
    function getUserCurrency() {
      // Try to detect from timezone or locale
      ...
      // Nigeria
      if (timezone.includes('Lagos') || locale.startsWith('en-NG')) {
        return 'NGN';
      }
      // Default to GBP for rest of world (UK and others)
      return 'GBP';
    }
    
    const userCurrency = getUserCurrency();
    const currencySymbols = { GBP: '£', EUR: '€', USD: '$', NGN: '₦' };
    ...
    function resetOffer() {
      ...
      ['business', 'company', 'enterprise'].forEach(function(plan) {
    ...
    function updateAllTotals() {
      ['business', 'company', 'enterprise'].forEach(updatePlanTotal);
    }
  ...
  function showEntitySelectModal(userId, preselectedPlan) {
    ...
    if (!entitySelectModal || !entitySelectType) {
      ...
      createBusinessForUser(userId, preselectedPlan || 'Company');
    ...
    entitySelectType.value = preselectedPlan || 'Company';
  ...
  (function initEntitySelectModal() {
    ...
    entitySelectConfirm.addEventListener('click', function() {
      ...
      const entityType = entitySelectType ? entitySelectType.value : 'Company';
   ...
   * Add validation handlers for all file upload inputs
   */
  (function initFileUploadValidation() {
    const fileInputs = [
      { id: 'txnReceipt', name: 'Transaction Receipt' },
      { id: 'invReceipt', name: 'Inventory Receipt' },
      { id: 'assetReceipt', name: 'Asset Receipt' },
      { id: 'assetDisposalReceipt', name: 'Asset Disposal Receipt' }
    ];
